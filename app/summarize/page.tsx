              ...
              <table className="w-full min-w-[900px] border-collapse text-sm bg-card">
                <thead>
                  <tr className="border-b border-border/40 bg-muted/40">
                    ...
                    <th className="px-1 py-2.5 text-center text-[10px] font-semibold text-muted-foreground/80 whitespace-nowrap tracking-wider uppercase text-center" style={{ minWidth: "80px" }}>Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  {details.length > 0 ? (
                    details.slice(page * 100, page * 100 + 100).map((d, idx) => (
                      <tr key={d.id} className={`border-b border-border/30 transition-colors hover:bg-accent/40 ${idx % 2 === 0 ? "" : "bg-muted/10"}`}>
                        ...
                        <td className="px-1 py-2.5 text-center text-center" style={{ minWidth: "80px" }}>
                          <div className="flex items-center justify-center gap-1">
                            <button onClick={() => setActiveDetail(d)}
                              className="rounded-md px-3 py-1 text-[10px] font-semibold text-foreground border border-border bg-background hover:bg-accent shadow-sm transition-colors whitespace-nowrap" style={{ minWidth: "58px" }}>
                              Detail
                            </button>
                            <button onClick={() => setEditDetail({ id: d.id.split("_").slice(1).join("_"), table: d.kategori === "Binding" ? "binding_tickets" : d.kategori === "GNO/REGFAIL" ? "gno_tickets" : d.kategori === "OG NOK" ? "ognok_tickets" : "routing_tickets", nomor_tiket: d.noTiket || null, no_service: d.noService, sto_lama: d.stoLama || null, sto_baru: d.stoBaru || null, domain: d.domain, alasan_binding: d.alasanBinding, jenis: d.jenis })}
                              className="flex-shrink-0 inline-flex items-center justify-center rounded-lg border border-border bg-background p-1.5 text-muted-foreground shadow-sm transition-all hover:border-blue-500/40 hover:text-blue-400 hover:bg-blue-500/5">
